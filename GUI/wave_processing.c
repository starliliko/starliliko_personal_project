...
void generate_sine_wave(float freq, float amp, float offset, float sample_interval, 
    ...
    if (!buffer || !phase) return;
    
    float phase_step = 2 * M_PI * freq * sample_interval;
    
    for (uint16_t i = 0; i < buf_len; i++) {
        buffer[i] = amp * sin(*phase) + offset;
        *phase += phase_step;
...
void generate_square_wave(float freq, float amp, float offset, float duty_cycle,
    ...
    if (!buffer || !phase || duty_cycle < 0.0f || duty_cycle > 1.0f) return;
    
    float phase_step = 2 * M_PI * freq * sample_interval;
    float threshold = 2 * M_PI * duty_cycle;
    
    for (uint16_t i = 0; i < buf_len; i++) {
        buffer[i] = (*phase < threshold) ? (amp + offset) : (-amp + offset);
        *phase += phase_step;
...
void generate_triangle_wave(float freq, float amp, float offset, float sample_interval,
    ...
    for (uint16_t i = 0; i < buf_len; i++) {
        // 三角波公式：在0到π之间从-1线性上升到1，在π到2π之间从1线性下降到-1
        if (*phase < M_PI) {
            buffer[i] = amp * (2 * (*phase / M_PI) - 1) + offset;
        } else {
            buffer[i] = amp * (3 - 2 * (*phase / M_PI)) + offset;
        }
